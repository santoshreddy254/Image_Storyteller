...
tf.compat.v1.flags.DEFINE_string("input_files", None,
...
tf.compat.v1.flags.DEFINE_string("vocab_file", "",
...
tf.compat.v1.flags.DEFINE_string("output_dir", None, "Output directory.")

tf.compat.v1.flags.DEFINE_integer("train_output_shards", 100,
                        "Number of output shards for the training set.")

tf.compat.v1.flags.DEFINE_integer("validation_output_shards", 1,
                        "Number of output shards for the validation set.")

tf.compat.v1.flags.DEFINE_integer("num_validation_sentences", 50000,
                        "Number of output shards for the validation set.")

tf.compat.v1.flags.DEFINE_integer("num_words", 20000,
                        "Number of words to include in the output.")

tf.compat.v1.flags.DEFINE_integer("max_sentences", 0,
                        "If > 0, the maximum number of sentences to output.")

tf.compat.v1.flags.DEFINE_integer("max_sentence_length", 30,
                        "If > 0, exclude sentences whose encode, decode_pre OR"
                        "decode_post sentence exceeds this length.")

tf.compat.v1.flags.DEFINE_boolean("add_eos", True,
...
def _build_vocabulary(input_files):
  ...
  for input_file in input_files:
    ...
    for sentence in tf.io.gfile.GFile(input_file):
  ...
  with tf.io.gfile.GFile(word_counts_file, "w") as f:
...
def _process_input_file(filename, vocab, stats):
  ...
  for successor_str in tf.io.gfile.GFile(filename):
