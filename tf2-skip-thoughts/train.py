...
if __name__ == "__main__":
    ...
    global_batch_size = int(batch_size_per_gpu * strategy.num_replicas_in_sync)
    ...
    with open('/home/smuthi2s/perl5/NLP/Image_Storyteller/tf2-skip-thoughts/data.txt', 'r') as in_file:
        total_sentences = in_file.read().splitlines()
    ...
    # vocab_size = len(self.tokenizer.word_counts)+1
    print(encoded_data)
    ...
    # dataset = tf.data.Dataset.from_tensor_slices((encoded_data, lengths))
    val_size = int(validation_size * len(encoded_data))
    print('Validation size: {}'.format(val_size))

    if val_size > 0:
        val_dataset = train_batches
